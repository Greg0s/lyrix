...
It stands in for `npm run dev:all` for that session: same address, same API, real scores and real close words. The table is rebuilt on each start (a few seconds), so a change to the scoring shows up on the next run, and it is kept in `worker/.wrangler/debug-state` — delete that directory to forget every table loaded there. `npm run dev:debug -- --help` lists the options: `--model` to name a model, `--table` to play one you already built, `--reveal` to show the hidden words faintly the way `npm run dev:all` does.
